...
void SysMtgsThread::OnStart()
{
	...
	m_RingBufferIsBusy.store(false, std::memory_order_release);

	m_QueuedFrameCount.store(0, std::memory_order_release);
	m_VsyncSignalListener.store(false, std::memory_order_release);
...
void SysMtgsThread::PostVsyncStart()
{
	...
	//Console.WriteLn( Color_Blue, "(EEcore Sleep) Vsync\t\tringpos=0x%06x, writepos=0x%06x", m_ReadPos, m_WritePos );
...
class RingBufferLock {
	...
	RingBufferLock(SysMtgsThread& mtgs)
		: m_lock1(mtgs.m_mtx_RingBufferBusy),
		  ...
		  m_mtgs(mtgs) {
		m_mtgs.m_RingBufferIsBusy.store(true, std::memory_order_release);
	}
	virtual ~RingBufferLock() throw() {
		m_mtgs.m_RingBufferIsBusy.store(false, std::memory_order_release);
	}
	void Acquire() {
		m_lock1.Acquire();
		m_lock2.Acquire();
...
void SysMtgsThread::WaitGS(bool syncRegs, bool weakWait, bool isMTVU)
{
	...
	if (isMTVU || m_ReadPos.load(std::memory_order_acquire) != m_WritePos.load(std::memory_order_relaxed)) {
		SetEvent();
		RethrowException();
		for(;;) {
			...
			RethrowException();
			if(!isMTVU && m_ReadPos.load(std::memory_order_acquire) == m_WritePos.load(std::memory_order_relaxed)) break;
...
{
	if(!m_RingBufferIsBusy.load(std::memory_order_acquire))
		m_sem_event.Post();
...
void SysMtgsThread::SendDataPacket()
{
	...
	else if(!m_RingBufferIsBusy.load(std::memory_order_acquire))
	{
